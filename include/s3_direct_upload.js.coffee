...
$.fn.S3Uploader = (options) ->
  ...
  setUploadForm = ->
    $uploadForm.fileupload
      ...
      progress: (e, data) ->
        $uploadForm.trigger("s3_uploads_progress", [e])
        # if data.context
      ...
      formData: (form) ->
        ...
        key = $uploadForm.data("key").replace('{timestamp}', new Date().getTime()).replace('{unique_id}', @files[0].unique_id).replace('${ext}', @files[0].name.split('.').pop())
  ...
  build_content_object = ($uploadForm, file, result) ->
    ...
    if result # Use the S3 response to set the URL to avoid character encodings bugs
      ...
      content.filepath = $uploadForm.find('input[name=key]').val().replace('/${ext}', '')
